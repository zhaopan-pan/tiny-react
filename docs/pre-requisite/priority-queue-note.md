结合上一篇的概念，用二叉堆来解决查找中位数问题，也是帮助我们更容易理解Scheduler模块中优先级调度的相关代码
...
使用两个优先级堆(priority heap)，即一个大顶堆，存放小于中位数的值，以及一个小顶堆，存放大于中位数的值。这会将所有元素大致分为两半，中间的两个元素位于两个堆的堆顶。这样一来，要找出中间值就是小事一桩。
